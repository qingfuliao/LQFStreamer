...
namespace LQF
{
	...
	{
	public:
		FrameTimer(const TimerEvent& event, uint32_t ms, bool repeat)
			: eventCallback(event)
			, _interval(ms)
			, _isRepeat(repeat)
		{
		...
		void start(int64_t microseconds, bool repeat = false)
		{
			...
			do
			{
				std::this_thread::sleep_for(std::chrono::microseconds(microseconds*2 - elapsed));
	...
	private:
		...
		std::mutex _mutex;
		std::map<TimerId, std::shared_ptr<FrameTimer>> _timers;
		std::unordered_map<uint32_t, std::shared_ptr<FrameTimer>> _repeatTimers;
