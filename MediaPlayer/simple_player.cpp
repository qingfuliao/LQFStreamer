...
static bool g_quit = false;     // 当g_quit == true时程序退出
static bool g_pause = false;    // 当g_pause == true时画面暂停
static int  g_frame_rate = 25;    // 默认

static  char *s_picture_type[] =
{
...
// 					SDL_RenderPresent(renderer);
				if (!is_display_open)
				{
					if (sdl2_display.display_init() < 0)
					{
						return -1;
					}
				...
				if (is_display_open)
					...
					printf("sdl2 display open failed\n");
