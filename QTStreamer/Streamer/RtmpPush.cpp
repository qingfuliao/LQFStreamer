...
RtmpPush::RtmpPush(const int audio_samplerate, const int video_fps):
	Pusher(audio_samplerate, video_fps)
{
	...
	// 暂时认为一帧的采样点数固定为1024
	audio_pkt_queue_.reset(new LQFRingBuffer<AVPacket>(1000.0 * 1024 / audio_samplerate));		// 只存储一帧的数据
	// 
	video_pkt_queue_.reset(new LQFRingBuffer<AVPacket>(video_fps));		// 只存储一帧的数据
}
...
void RtmpPush::SetAudioConfig(int profile, int samplerate, int channel)
{
...
bool RtmpPush::Connect(const char * url,int timeout)
{
