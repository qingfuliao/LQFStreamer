...
int AACEncode::Init(const int sample_rate, const int channels, const int bit_rate, const int profile_aac)
{
	AACENC_ERROR ret;
	...
	AACENC_InfoStruct *fdk_aac_info_ = (AACENC_InfoStruct *)malloc(sizeof(AACENC_InfoStruct));
	// 获取编码器信息
...
int AACEncode::Encode(const uint8_t * input, const int input_len, uint8_t * output, int &output_len)
{
	AACENC_ERROR ret;
...
int AACEncode::GetPcmFrameLength()
{
