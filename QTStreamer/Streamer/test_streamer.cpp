...
int main_rtmp_push_test()
{
	LogInit("main_rtmp_push_test.txt");

	LogDebug("main_rtmp_push_test");
	...
	{
		Sleep(1000 * 5);
		if (LQF::AVTimer::GetInstance()->GetTime() > 60 * 1000)
		{
			break;
		}
...
int main_mp4_reord_test()
{
	LogInit("mp4_reord_test.txt");
	LogDebug("main_mp4_reord_test");
	MediaCenter media_center;
...
int main_timer_test()
{
	std::cout << "main_rtp_push_video_test" << std::endl;
...
int main_rtp_push_video_test()
{
	LogInit("main_rtp_push_video_test.txt");
	LogDebug("main_rtp_push_video_test");
...
int test_streamer(void)
{
	InitSockets();

	std::cout << "main_mp4_reord_test" << std::endl;
	main_mp4_reord_test();
// 	main_rtmp_push_test();
// 	main_timer_test();
// 	main_rtp_push_video_test();
	CleanupSockets();
	system("pause");
	return 0;
}
